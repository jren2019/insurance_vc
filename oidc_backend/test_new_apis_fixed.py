#!/usr/bin/env python3
"""
Test script for the new issue_credential and verify_credential APIs with unique IDs
"""
import requests
import json
import time
...
def generate_unique_credential_id():
    """Generate a unique credential ID"""
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    unique_part = uuid.uuid4().hex[:8].upper()
    return f"TEST-{timestamp}-{unique_part}"
...
def test_issue_credential():
    ...
    unique_id = generate_unique_credential_id()
...
def main():
    print("=" * 60)
    print("Testing New OIDC APIs with Unique IDs")
    print("=" * 60)
